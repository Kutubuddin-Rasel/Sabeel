...
open class FakeTasbihRepository : TasbihRepository {
    ...
    val _streak = kotlinx.coroutines.flow.MutableStateFlow<com.kitalonlabs.sabeel.domain.model.Streak?>(null)
    override val streak: kotlinx.coroutines.flow.Flow<com.kitalonlabs.sabeel.domain.model.Streak?> = _streak

    var delayMs: Long = 0
    var setDhikrCalls: Int = 0

    override fun getDailyTargetFlow(date: String, dhikrKey: String): kotlinx.coroutines.flow.Flow<com.kitalonlabs.sabeel.domain.model.DailyTarget?> =
    ...
    override suspend fun setDhikr(key: String, targetOverride: Int?) {
        if (delayMs > 0) kotlinx.coroutines.delay(delayMs)
        setDhikrCalls++
        _activeDhikr.value = com.kitalonlabs.sabeel.data.repository.applyTargetOverride(

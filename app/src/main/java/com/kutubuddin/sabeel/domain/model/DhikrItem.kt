...
data class DhikrItem(
    ...
    val searchableWords: List<String> = run {
        ...
        com.kutubuddin.sabeel.domain.util.FuzzySearchUtils.normalize(allText)
            .split("\\s+".toRegex())
    ...
    fun searchScore(query: String): Int {
        val queryWords = com.kutubuddin.sabeel.domain.util.FuzzySearchUtils.normalize(query)
            .split("\\s+".toRegex())
            .filter { it.isNotBlank() }
        ...
        for (qWord in queryWords) {
            ...
            for (tWord in searchableWords) {
                if (tWord == qWord) {
                    ...
                    val dist = com.kutubuddin.sabeel.domain.util.FuzzySearchUtils.levenshteinDistance(qWord, tWord)
